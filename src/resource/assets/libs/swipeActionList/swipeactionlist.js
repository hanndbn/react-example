...
ListView.prototype.parserData = function(dataInput) {
    ...
    for (var i = 0; i < lengthData; i++) {
        ...
        if (items !== undefined) {
            ...
            if (this.numberRecords > 5 && this.numberRecords == (i + 1) && this.option == "default") {
                break;
            }
        }
    }
...
ListView.prototype.parserDataGroup = function(dataInput, dataTempRefresh, vGroup) {
    ...
    for (var i = 0; i < lengthData; i++) {
        ...
        if (this.fieldGroup == undefined) fGroup = vGroup;
        //convert one
...
ListView.prototype.createButton = function(id, dictButton) {
    ...
    divButton.innerHTML = "<div class='p-btn-" + dictButton.type + "' " + styleButton + "><span class='" + dictButton.icon + "'></span></div>";
...
ListView.prototype.showList = function(elementParent) {
            ...
            if (this.option == "group") {
                ...
                for (var prop in this.data) {
                    if (prop.trim() != "") {
                        ...
                        z++;
                    }else if(this.data[prop].length>0){
                         
                        var ulListView = document.createElement("ul");
                        for (var i = 0; i < this.data[prop].length; i++) {
                            var item = this.createItem(i, this.data[prop][i], z,true);
                            ulListView.appendChild(item);
                        }
                        ulListView.id = "p-ul-" + z;
                        if (this.showGroup != true) {
                            ...
                            ulListView.style.borderRadius = "10px 10px 10px 10px";
                            ulListView.style.opacity = 1;
                            ulListView.style.visibility = "inherit";
                            ulListView.style.height = "auto";
                            ulListView.style.transition = "ease 0.8s";
    ...
    //    }
    //}
ListView.prototype.createItem = function(i, data, idUl,isGroupFalse) {
    ...
    li.addEventListener("click", function(e) {
		/*HaiNM*/
		if(CONST_DESKTOP_MODE && !checkScreenisMobilePX() == true){
			if (flagClickDot == false){
				e = e || window.event;
				...
				if (bowser.name=='Firefox'){
					if (e.originalTarget.nodeName==='LI') {
						...
						el = e.originalTarget.parentElement;
						el.addEventListener("touchmove", touchMove, true);
						el.addEventListener("touchend", touchEnd, true);
						tempIdPrevNode = e.originalTarget.parentElement.id;
					}
				}else{
					if (e.srcElement.nodeName === 'LI') {
						...
						el.addEventListener("touchmove", touchMove, true);
						el.addEventListener("touchend", touchEnd, true);
						tempIdPrevNode = e.srcElement.parentElement.id;
					}
				}
			}	
		}
		if (bowser.name=='Firefox' || bowser.name=='IE'){
			logInfo("content :" + e.target.offsetParent.className,"swipeactionslist 641");
			if(e.target.offsetParent.className =="p-list-action"){
				try {
					e.preventDefault();
					return;
				}catch(e){
			...
			logInfo("content :" + e.toElement.offsetParent.className,"swipeactionslist 641");
			if(e.toElement.offsetParent.className =="p-list-action"){
				try {
					e.preventDefault();
					return;
				}catch(e){
		...
		/*End*/
        var parentId = li.parentNode.parentNode.id;
        if (tempListViewGlobal[parentId].doubleClickItem != null && typeof tempListViewGlobal[parentId].doubleClickItem === 'function') {
            tempListViewGlobal[parentId].doubleClickItem(li);
        }
    }, true);
    return li;
}
...
function touchEnd(event) {
	...
	/*HaiNM - cho phép vuốt sang trái 1 lần click*/
    // if (flagClickDot == true && flagTouchStart == true) {
        // flagClickDot=false;
    // }
	/*End*/
    var endTouch = {
    ...
    if (typeof node !== 'undefined' && node !== null) {
        ...
        if (move.x < start.x) {
            if (tmpRight < 0 || (flagTouchStart == true && flagTouchMove == false && flagTouchEnd == true)) {
                node.style.width = maxWith + 'px';
                node.style.right = -maxWith + 'px';
                node.style.display = 'block;';
            }
