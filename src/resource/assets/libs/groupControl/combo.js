...
function Combo(opt){
   ...
   this.accountNameTitle = opt.accountNameTitle;
...
Combo.prototype.show = function (idParent){
    var combo = this;
    ...
    container.innerHTML = renderDataCombo(this.accountNameTitle,this.accountName,this.accountNumber,this.accountBalance,this.paddingIcon);
...
Combo.prototype.refresh = function (opt){
    this.accountName = opt.accountName;
    this.accountNumber = opt.accountNumber;
    this.accountBalance = opt.accountBalance;
...
Combo.prototype.getByIndex = function(index){
...
function fillData(containerId,accountName, accountNumber,accountBalance){
    ...
    if(typeof accountBalance !== "undefined"){
        container.querySelector(".accountBalance").innerHTML = '<span>' + accountBalance + '</span>';
       // document.getElementById('accountBalance').childNodes[0].innerHTML = accountBalance;
    }
}
function renderDataCombo(accountNameTitle, accountName, accountNumber,accountBalance,paddingIcon){
    var html = '<div style="width:98%">';
    if(typeof accountName !== undefined && accountName.length > 0 && accountName!=""){
        if(accountNameTitle!=undefined && accountNameTitle!=""){
             html+='<div class="accName"><span>'+ accountNameTitle +'</span></div><div class="accountName"><span>'+accountName+'</span></div>';
        }else{
             html+='<div class="accountName" style="width:100%;"><span>'+accountName+'</span></div>';
        }
    ...
    if(typeof accountBalance !== undefined && accountBalance.length > 0 && accountBalance!=""){
        html+='<div class="accName"><span>'+CONST_STR.get('ACCOUNT_AVAILABLE_BALANCE')+'</span></div><div class="accountBalance"><span>'+accountBalance+'</span></div>';
    }
    html+='</div>';
    html+='<div style="float:right;" id="arrow"><span class="icon-movenext input-group-symbol"></span></div>';
    return html;
}