...
var NotificationDialog = function (params) {
    var dialog = this;
    var DIALOG_FULL = 1;
    ...
    dialog.showDialogFull = function (params) {
        ...
        var modal = document.getElementById('myModalFullDialog');
        ...
        if (modal != undefined) {
            ...
            var bgDialog = document.getElementById('dialogFullBackground');
    ...
    dialog.onCreateDialog = function () {
        ...
        if (typeDialog != undefined && typeDialog == DIALOG_MESSAGE) {
            ...
            var content = "<div id='myModalDialog' class='modaldialogContent'><div class='eb-modal-content'><div class='eb-modal-header'><span id ='eb_modal_close' class='eb-modal-close'>&times</span><h4>" + title + "</h4></div>" +
                ...
                "<div class='divider-modal-dialog'></div><div class='eb-modal-footer'>" +"<button id='eb_dialog_cancel' style='width: 36%' onClick='closeAlertApp(false)'>" +cancel + "</button><button id='eb_dialog_agree' style='width: 36%' onClick='closeAlertApp(true)'>" + agree + "</button></div></div></div>";
    ...
    dialog.removeDialogFull = function () {
        var item = document.getElementById("myModalFullDialog");
        var parent = document.getElementById(params.parentNote);
        if (item != undefined && parent != undefined) {
            if(isDescendant(parent,item)){
                try{
