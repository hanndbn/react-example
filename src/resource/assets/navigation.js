    ...
    if (isInit || ((navCachedPages[page] == undefined) || (navCachedPages[page] == null)) || (currentPage == navController.getDefaultPage()) || tmpStatusNoCachePage) {
        ...
        // code for Chrome, Firefox, Opera, etc.
            ...
            if(navigator.userAgent.toLocaleLowerCase().indexOf('firefox') != -1){
                xmlAsString = xmlAsString.replace(/&lt;/gi, "<");
                xmlAsString = xmlAsString.replace(/&gt;/gi, ">");
                xmlAsString = xmlAsString.replace(/&amp;#60;/gi, "&lt;");
                xmlAsString = xmlAsString.replace(/&amp;#62;/gi, "&gt;");
            }
...
function applyVerticalScrollPage(forced, inGapsize) {
    ...
    var styles =
        '.main-layout .main-layout-vertical-slideview { height: ' + (clientHeight - 225 - inGapsize) + 'px; }';
    var style = document.createElement('style');
        ...
        if (mainViewContent) {
            ...
            if(gModeScreenView == CONST_MODE_SCR_SMALL) {
               ...
               mainViewContent.style.height =  mainview.offsetHeight - 60 - actionbar.getHeight() + 'px';
                /*mainViewContent.style.height =  mainview.offsetHeight - 70 - actionbar.getHeight() + 'px';*/
            ...
            if(gModeScreenView == CONST_MODE_SCR_SMALL){
                ...
                mainViewVerticalSlide.style.height = mainview.offsetHeight - 60 - actionbar.getHeight() + 'px';
...
function checkTransferPage() {
    var navTransferPages = ['transfer/transfer-card-create-scr', 'transfer/transfer-fpts-create-scr', 'transfer/transfer-id-create-scr','transfer/transfer-inter-bank','transfer/transfer-local-create-scr','gold/gold-transfer-create-src'];
    var length = navArrayScr.length;
    for(var i=0;i<length;i++){
        var pageContent = navArrayScr[i];
         if(navTransferPages.indexOf(pageContent)>=0){
             return true;
         }
    }
...
function resizeMainViewContentWhenKeyboardShow() {
    if(navArrayScr!=undefined){
        var length = navArrayScr.length;
         var mainViewContent = document.getElementById('mainViewContent');
		 if(mainViewContent==undefined){
             mainViewContent =  document.getElementById('mainViewVerticalSlide');
         }
        if(Environment.isAndroid() && navArrayScr[length-1]!="login-scr"){//check xem co phai dung Android khong va page phai khac page login moi rezise
          
            var mainViewVerticalSlide = document.getElementById('mainViewVerticalSlide');
            var navBottomBar = document.getElementById('navBottomBar');
            var mainview = document.getElementById('mainview');
            var actionbar = document.getElementById('navActionbar');
            var heightMain = mainview.offsetHeight;
            var heightBottom = navBottomBar.offsetHeight;
            var heightMainContent = mainViewContent.offsetHeight;
            var heightActionBar = actionbar.offsetHeight;
            
            var heightTemp = mainViewContent.offsetHeight;
            var height= (window.innerHeight || document.documentElement.clientHeight);
            var heightTempMain = (height+heightBottom+heightActionBar-5);
            
			if((Math.abs(height-heightMainContent))>100){
						if(heightTemp>heightTempMain && height<heightMain){
							mainViewContent.style.height = (height-5)+'px';
							mainViewContent.style.top = '146px';
						}
						else{
							mainViewContent.style.height = (height-heightBottom-heightActionBar+5)+'px';
							mainViewContent.style.top=heightActionBar+"px";
						}
			}
        }
         else{
			 ...
			 if(gIsLogin){
				 if((CONST_DESKTOP_MODE && !checkScreenisMobilePX())){
						...
						if(heightMainContent>631){
						 ...
						 logInfo((heightActionBar + heightMainContent-100),"navigation:2565");
               ...
               var divID = document.getElementById("tabHost");
                ...
                var marginTopNone = Math.abs((currentClientHeight- h)/2);
                var marginLeftNone = Math.abs((currentClientWidth - w)/2);
                console.log("marginTopNone " + marginTopNone +" marginLeftNone " + marginLeftNone);
             
                 var noteModel = document.getElementById("myModalFullDialog");
                 var noteModelContent = document.getElementById("id-eb-modalfull-content");
                 if(noteModel == undefined){
                     noteModel = document.getElementById("myModalFullDialog-second");
                 }
                 if(noteModel != undefined){
                    //  noteModel.style.top = marginTopNone +"px !important";
                    //  noteModel.style.left = marginLeftNone +"px !important";
                     noteModel.style.setProperty ("top", marginTopNone + "px", "important");
                     noteModel.style.setProperty ("left", marginLeftNone + "px", "important");
                 }
                
                 if(noteModelContent != undefined){
                    //  noteModelContent.style.top = marginTopNone +"px !important";
                    //  noteModelContent.style.left = marginLeftNone +"px !important";
                      noteModelContent.style.setProperty ("top", marginTopNone + "px", "important");
                     noteModelContent.style.setProperty ("left", marginLeftNone + "px", "important");
                 }
        ...
        if(navArrayScr[navArrayScr.length - 1]=="homepagexsl/home-page-recent" || navArrayScr[navArrayScr.length - 1]=="homepagexsl/home-page-manage-finance" ||  navArrayScr[navArrayScr.length - 1]=="homepagexsl/cus-prop-info-scr" || navArrayScr[navArrayScr.length - 1]=="homepagexsl/homepage-dynamic-scr"){
            if((CONST_DESKTOP_MODE && !checkScreenisMobilePX())){
                navController.getActionBar().hideNavHeaderBar();
                updateMainContentWidth(window.innerWidth, window.innerHeight);
                resizeMainViewContent(navArrayScr[navArrayScr.length - 1]);
                updateViewForDesktop();
                mainViewContent.style.top = 44 + 'px';
            }
            else{
                updateMainContentWidth(window.innerWidth, window.innerHeight);
                resizeMainViewContent(navArrayScr[navArrayScr.length - 1]);
                ...
                mainViewContent.style.top = 161 + 'px';
                var icontpbank = document.getElementById('nav.icon.tpbank');
                icontpbank.style.display = 'block';
                var iconthome = document.getElementById('nav.btn.home');
                iconthome.style.display = 'block';
            }
        }
